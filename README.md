...
Live at [profitofexile.top](https://profitofexile.top) · 190 desktop installs + web-comparator users · strong retention (low-20%s daily active-session-to-install ratio) · ~72k source LOC · Go / SvelteKit / Tauri (Rust) — built across stacks I had no prior expertise in. Active Discord support where I triage user reports via log analysis and fold recurring issues into known-issues docs.

**📚 Education CRM** *(client project, in production)*
Multi-tenant SaaS for a Polish education client — 4,190 tests (2,674 unit / 459 integration / 699 functional / 201 smoke / 157 E2E), PHPStan level 8, full audit in ~7 min — PHP 8.4 / Symfony 7.4. CQRS, event-driven architecture, GDPR compliance. In daily use: 340 active groups, 3,227 prospects under management. Client reports the built-in SMS + email automation alone has freed hours of staff time each week vs their prior manual workflow.

**🏗️ Pipeforge** *(proprietary, two repos: plugin + MCP backend, demo available on request)*
Personal AI development infrastructure — Claude Code orchestration plugin + self-authored PostgreSQL MCP backend. Built to power my own solo workflow across 2 machines and 3 parallel worktrees on client work. 31 commands, 28 skills, 20 agents; multi-agent parallel execution with dependency resolution. 64 MCP tools across 31 tables backing cross-session state. Team collaboration patterns architected in from day one (seed-derived per-project agents updated from team-contributed MCP knowledge; cursor-based codemap synthesis safe for concurrent runs). Currently solo by choice, not architectural limit; ~1 week of docs/onboarding from team-shareable.
